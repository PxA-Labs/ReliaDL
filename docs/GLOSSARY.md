...
| **Coroutine** | A Python async function that can be suspended and resumed. ReliaDL uses coroutines for non-blocking download workers. |
...
| **ETag** | An HTTP response header containing a version identifier for the resource. Used by ReliaDL to detect if the file changes on the server during a download. |
